...
BaseClient* RenvClient::GetClient()
{
	...
	assert(!mRmtSID.empty());
	string resp;
	bool rr = client->Request("EnvProvider", "AttachEnv,1," + mRmtSID, resp);
	if (rr) {
	    res = client;
	    mClients.push_back(client);
	} else {
...
bool DaaPxMgr::IsCached(const string& aContext) const
{
...
bool DaaPxMgr::Request(const string& aContext, const string& aReq, string& aResp)
{
    bool res =  mRenvClient.Request(aContext, aReq, aResp);
    if (!res) {
...
{ 
    MIface* res = NULL;
    string resp;
    bool rres = mMgr->Request(mContext, aCallSpec, resp);
    if (rres) {
