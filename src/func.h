#ifndef __FAP5_FUNC_H
#define __FAP5_FUNC_H
...
class Func
{
	...
	class Host {
	    public: 
	    virtual MDVarGet* GetInpIc(int aInpId, int aIcId = 0) = 0;
	    virtual int GetInpIcCount(int aInpId) = 0;
...
template <class T> inline const T* Func::GetInpData(int aInpId, const T* aData)
{
    ...
    int icsCount = mHost.GetInpIcCount(aInpId);
    if (icsCount == 0) {
