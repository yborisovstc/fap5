...
MIface* Elem::MElem_getLif(TIdHash aId)
{
    return nullptr;
}
...
MIface* Elem::MNode_getLif(TIdHash aId)
{
    MIface* res = nullptr;
    if (res = checkLif2(aId, mMElem));
    else if (res = checkLif2(aId, mMParent));
    else res = Node::MNode_getLif(aId);
    return res;
}

GUri Elem::parentUri() const
{
    string res;
    GUri uri;
...
void Elem::mutate(const ChromoNode& aMut, bool aChange /*EFalse*/, const MutCtx& aCtx, bool aTreatAsChromo, bool aLocal)
{
	...
	PFL_DUR_STAT_REC(PEvents::EDurStat_MutCad);
        Node::mutate(mut, aChange, aCtx, aTreatAsChromo, aLocal);
    } else {
        Node::mutate(aMut, aChange, aCtx, aTreatAsChromo, aLocal);
    }
}
...
MIface* Elem::MParent_getLif(TIdHash aId)
{
    MIface* res = nullptr;
    return res;
}
...
MIface* Elem::MChild_getLif(TIdHash aId)
{
    MIface* res = nullptr;
    return res;
}
